...
class _MyAddressScreenState extends State<MyAddressScreen> {
  ...
  Future<void> saveUserData() async {
    final http.Response response =
        await http.post(Uri.parse(kUpdateAddressUrl), body: {
      "email": userApi.email,
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      if (data.toString() == "SUCCESS") {
        ...
        AlertBox.showSuccessBox(
            context, 'Address updated successfully', 'SUCCESS');
  ...
  Future<void> getLocationFromAddress() async {
    final query =
        "$houseName, $streetName, $cityName, $stateName $postalCode, $countryName";
